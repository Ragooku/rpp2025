...
class Machine(models.Model):
    ...
    last_maintenance = models.DateField(verbose_name="Дата включения", null=True, blank=True)
    installation_date = models.DateField(verbose_name="Дата выключения")
...
class MachineSpecification(models.Model):
    ...
    max_vibration = models.DecimalField(max_digits=6, decimal_places=2, verbose_name="Макс. свет")
...
class MachineReading(models.Model):
    reading_number = models.AutoField(primary_key=True, verbose_name="№ светофора")
    machine = models.ForeignKey(Machine, on_delete=models.CASCADE, verbose_name="Светофор")
    timestamp = models.DateTimeField(verbose_name="Дата и время")
    vibration_frequency = models.DecimalField(max_digits=6, decimal_places=2, verbose_name="Кол-во машин в пробке")
    power_consumption = models.DecimalField(max_digits=6, decimal_places=2, verbose_name="Кол-во проехало")
    temperature = models.DecimalField(max_digits=6, decimal_places=2, verbose_name="")
